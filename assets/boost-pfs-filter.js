...
general: {
  ...
  enableOTP: true,
  filterTreeMobileStyle: '', // 'style2', 'style3',
...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  // Add first available variant ID
  itemHtml = itemHtml.replace(/{{itemVarID}}/g, buildVarID(data));

  // Add OOS status disabled on atc button
  itemHtml = itemHtml.replace(/{{itemStatus}}/g, buildStatus(data));

  // Add OOS copy on atc button
  itemHtml = itemHtml.replace(/{{btnCopy}}/g, buildBtnCopy(data));

  var images_first = '';
  var images_second = '';
  var propOwn = Object.getOwnPropertyNames(data.images)
  if (propOwn.length > 0) {
    images_first = data.images[1].replace(/\.(png|jpg)/g, '_280x280.$1');
    images_second = data.images[1].replace(/\.(png|jpg)/g, '_560x560.$1');
  }

  itemHtml = itemHtml.replace(/{{imageOne}}/g, images_first );
  itemHtml = itemHtml.replace(/{{imageOne2x}}/g, images_second);

  if (propOwn.length > 1) {
    itemHtml = itemHtml.replace(/{{imageTwo}}/g, data.images[2].replace(/\.(png|jpg)/g, '_280x280.$1'));
    itemHtml = itemHtml.replace(/{{imageTwo2x}}/g,  data.images[2].replace(/\.(png|jpg)/g, '_560x560.$1') );
    itemHtml = itemHtml.replace(/{{flip}}/g, ' product--hover-swap');
  } else if (propOwn.length > 0){
      itemHtml = itemHtml.replace(/{{imageTwo}}/g, data.images[1].replace(/\.(png|jpg)/g, '_280x280.$1') );
      itemHtml = itemHtml.replace(/{{imageTwo2x}}/g,  data.images[1].replace(/\.(png|jpg)/g, '_560x560.$1') );
      itemHtml = itemHtml.replace(/{{flip}}/g, '');
  }
  ...
  if (typeof Integration === 'undefined' ||
	...
	itemHtml = itemHtml.replace(/{{ATC_btn}}/g, buildATC(data));
...
function buildATC( data ) {
  var button_html;
  var disabled = '';

  if ( data.variants.length > 1 ) {
    button_html = '<a href="'+Utils.buildProductItemUrl(data)+'" class="btn btn--solid">Select Options</a>'
  } else {
    if ( data.available != true ) {
      disabled = 'disabled="disabled"';
    }
    button_html = '<button class="btn btn--solid btn--atc" '+disabled+' data-id="'+data.variants[0].id+'" aria-label="Add '+data.title+' to your cart">Add To Cart</button>'
  }
...
function buildVarID(data) {
  var html = '';

  html = data.variants[0].id;
...
function equalHeight(data) {
  ...
  if(typeof aspect_ratio === "undefined") return;
